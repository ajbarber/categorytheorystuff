...
Take $\theta(S) = A - g\left(B \setminus f\left(S\right)\right)$. Then $S_1 \subseteq S_2 \implies \theta(S_1) \subseteq \theta(S_2)$. Since $f, g$ are injective, and hence $\theta$ is order preserving, we may apply the result in (a). Specifically, there exists $S$ such that $S = A - g\left(B \setminus f\left(S\right)\right) \implies g\left(B \setminus f\left(S\right)\right) = A \setminus S$.
