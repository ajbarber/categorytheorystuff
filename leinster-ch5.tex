...
Now we need a monic that is \textbf{not} regular. This is a monic that does not admit an equalizer with some $f \neq g$, as above. We know all regular monics in \textbf{Top} are the inclusion of a subspace. We also know that all injections are monics. So we simply need to find an injection between spaces which is not the inclusion of a subspace.
So choose $m$ with an domain that is equipped with a finer topology than the subspace topology of $B$. Under a finer topology for $A$ we'll have at least one open set in $A$ mapping to a closed set in $m(A)$. However this means $g$ is not continuous at this point, so no such pair of mappings exist.
